...
tday1=datestr(datenum(tday1, 'dd/mm/yyyy'), 'yyyymmdd'); % convert the format into yyyymmdd.
...
tday2=datestr(datenum(tday2, 'dd/mm/yyyy'), 'yyyymmdd'); % convert the format into yyyymmdd.
...
longs=zscore<=-1; % buy spread when its value drops below 2 standard deviations.

shorts=zscore>=1; % short spread when its value rises above 2 standard deviations.

exits=abs(zscore)<=0.5; % exit any spread position when its value is within 1 standard deviation of its mean.
