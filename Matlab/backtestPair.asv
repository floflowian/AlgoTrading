function [ cumulPnl,sharpeTrainset,sharpeTestset,spread ,trainingset,testset ] = backtestPair( symbol1, symbol2, longStd, shortStd, exitStd,stopLoss,plots )
...
longs=zscore<=-longStd; %buy when spread is more that 2 std dev away from mean

shorts=zscore>=shortStd; %short when spread is more that 2 std dev away from mean

exits=(abs(zscore)<=exitStd | abs(zscore)>=stopLoss) ; %exit any position when spread comes within 0.5 std dev from mean
...
sharpeTrainset=sqrt(252)*mean(pnl(trainingset(2:end)))./std(pnl(trainingset(2:end)));
